...
|---|---|
| Figure 1, ST-NLR framework | `figures/artwork/FIG1_framework.svg`, shadow-free pure-vector PDF, and 1000 dpi PNG/TIFF; validator in `figures/prepare_figure1_svg.py` |
...
| Table 6, Cahn--Hilliard transfer | `results/paired_uncertainty.json` for the paired continued-static comparison; `results/cahn_hilliard.json` and `configs/experiment_protocols.yaml` for the system setup and rank-selection protocol |
...
| Table C.9, full fine-tuning | `results/full_finetuning.json` for the full-update reference and `results/static_rank_controls.json` for the final four-condition ST-NLR operating point |
...
| Figure E.7, adaptation-data budget under a fixed validation set | `results/data_budget.json`, `figures/make_summary_figures.py` |
